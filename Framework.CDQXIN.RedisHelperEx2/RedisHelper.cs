...
namespace Framework.CDQXIN.RedisHelperEx2
{
    /// <summary>
    /// redis帮助类
    /// </summary>
    public class RedisHelper
    {
        #region 连接信息
        //从配置文件中获取连接字符串  
        private static readonly string RedisHost = ConfigurationManager.AppSettings["RedisConnectionHost"];
        private static string RedisPath = ConfigurationManager.AppSettings["RedisConnectionHost"].ToString();
        private static readonly int RedisPort = Convert.ToInt32(ConfigurationManager.AppSettings["RedisConnectionPort"]);
        private static readonly string RedisPassWord = ConfigurationManager.AppSettings["RedisConnectionPassWord"];
        private static string redisHost = (string.IsNullOrEmpty(RedisPassWord) ? "" : (RedisPassWord + "@")) + RedisPath + ":" + RedisPort;
        public static PooledRedisClientManager Prcm = CreateManager(new string[] { redisHost }, new string[] { redisHost });
        ...
        //static   RedisClient redisClient = new RedisClient(RedisHost, RedisPort) { Password = RedisPassWord };
        #endregion

        private static RedisClient GetClient()
        {
            return new RedisClient(RedisHost, RedisPort) { Password = RedisPassWord };
        }
        ...
        /// <returns>操作是否成功</returns>  
        public static bool ItemSet<T>(string key, T t, int min = 5)
        {
            using (IRedisClient redis = Prcm.GetClient())
            {
                return redis.Set<T>(key, t, new TimeSpan(0, min, 0));
            }
        }
        public static bool ItemSet<T>(string key, T t, DateTime dt)
        {
            using (IRedisClient redis = Prcm.GetClient())
            {
                return redis.Set<T>(key, t, dt);
            }
        }
        ...
        public static bool Add<T>(string key, T value, int expiretime)
        {
            using (IRedisClient redis = Prcm.GetClient())
            {
                return redis.Set<T>(key, value, new TimeSpan(0, 0, expiretime));
            }
        }
        ...
        /// <param name="expiretime">expiretime(单位秒)</param>
        /// <returns></returns>
        public static bool Add(string key, string value, int expiretime)
        {
            using (IRedisClient redis = Prcm.GetClient())
            {
                return redis.Set(key, value, new TimeSpan(0, 0, expiretime));
            }
        }
        ...
        /// <param name="expiretime">过期时间（单位秒）</param>
        /// <returns></returns>
        public static bool SetExpireTime(string key, int expiretime)
        {
            try
            {
                using (IRedisClient redis = Prcm.GetClient())
                {
                    return redis.ExpireEntryIn(key, new TimeSpan(0, 0, expiretime));
                }
            }
            catch
            {
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static long IncrementValue(string key, int count)
        {
            try
            {
                using (IRedisClient redis = Prcm.GetClient())
        ...
        public static void AddItemToSet(string setId, string val)
        {
            using (IRedisClient redis = Prcm.GetClient())
            {
                redis.AddItemToSet(setId, val);
            }
        }
        /// <summary>
        /// 从集合中随机获取项
